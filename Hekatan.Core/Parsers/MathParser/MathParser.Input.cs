    ...
    public partial class MathParser
    {
        private sealed class Input
        {
            ...
            private TokenTypes GetCharType(char c) => c switch
            {
                ...
                '÷' or '⦼' or
                '∧' or '∨' or
                '⊕' or '←' => TokenTypes.Operator, // v7.6.1: Added ← for outer scope assignment
            ...
            {
                // v7.6.1: Check if expression has ← operator for outer scope assignment
                bool hasOuterAssign = false;
                bool isFirstVariable = true;
                foreach (var token in tokens)
                {
                    if (token.Type == TokenTypes.Operator && token.Content == "←")
                    {
                        hasOuterAssign = true;
                        break;
                    }
                ...
                foreach (var token in tokens)
                {
                    ...
                    {
                        // v7.6.1: For ← operator, first variable uses outer scope
                        if (hasOuterAssign && isFirstVariable)
                        {
                            GetVariableFromOuterScope(vt);
            ...
            /// <summary>
            /// v7.6.1: Get variable from outer scope (skip the topmost local scope)
            /// Used for the ← (outer scope assignment) operator
            /// </summary>
            private void GetVariableFromOuterScope(VariableToken vt)
            {
                var s = vt.Content;
                var isFirst = true;
                foreach (var variables in _localVariables)
                {
                    // Skip the topmost (most local) scope
                    if (isFirst && _localVariables.Count > 1)
                    {
                        isFirst = false;
                        continue;
                    }
                ...
                // If not found in any outer scope, create in global scope
                var globalVariables = _variables;
                var newVar = new Variable();
                globalVariables.Add(s, newVar);
                vt.Variable = newVar;
            }
