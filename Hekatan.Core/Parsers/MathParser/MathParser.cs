...
namespace Hekatan.Core
{
    public partial class MathParser
    {
        ...
        public void Parse(ReadOnlySpan<char> expression, bool allowAssignment = true)
        {
            ...
            // v7.6.1: Preprocess shortcut <* to ← (outer scope assignment)
            if (expression.Contains("<*".AsSpan(), StringComparison.Ordinal))
                expression = expression.ToString().Replace("<*", "←").AsSpan();
