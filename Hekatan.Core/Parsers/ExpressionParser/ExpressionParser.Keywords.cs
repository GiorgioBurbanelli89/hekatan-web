...
namespace Hekatan.Core
{
    public partial class ExpressionParser
    {
        private enum Keyword
        {
            ...
            Local,
            Global,
            Const,  // v7.6.1: Variables constantes
            Pause,
            Input,
            Md,
            Read,
        ...
        /// <summary>
        /// Parse #const directive - defines constant variables (v7.6.1)
        /// Usage: #const PI = 3.14159
        ///        #const area(r) = PI*r^2
        /// Constants cannot be reassigned after definition.
        /// </summary>
        private void ParseKeywordConst(ReadOnlySpan<char> s)
        {
            // Extract expression after "#const" (6 characters)
            ReadOnlySpan<char> expression = s.Length > 6 ?
                s[6..].Trim() :
                [];

            if (expression.IsWhiteSpace())
            {
                AppendError(s.ToString(), "Missing expression after #const", _currentLine);
                return;
            }

            // Find the equals sign to separate variable name from value
            int eqIndex = expression.IndexOf('=');
            if (eqIndex < 1)
            {
                AppendError(s.ToString(), "Invalid #const syntax. Use: #const name = value", _currentLine);
                return;
            }

            var varName = expression[..eqIndex].Trim().ToString();
            var valueExpr = expression[(eqIndex + 1)..].Trim();

            // Check for function definition: #const f(x) = x^2
            int parenIndex = varName.IndexOf('(');
            bool isFunction = parenIndex > 0;
                ...
                if (_condition.IsSatisfied)
                {
                    try
                    {
                        if (isFunction)
                        {
                            ...
                            // Format: "name(params) = expression"
                            var funcDef = $"{varName} = {valueExpr}";
                            _parser.Parse(funcDef.AsSpan(), true);
                            _parser.Calculate();
                            // Note: Function constants need additional implementation
                            // to prevent redefinition
                        }
                        else
                        {
                            // Parse as variable constant
                            _parser.Parse(valueExpr);
                            _parser.Calculate();
                            var r = _parser.Result;
                            var u = _parser.Units;
                            IValue result;
                            if (r.IsReal)
                                result = new RealValue(r.Re, u);
                            else
                                result = new ComplexValue(r, u);
                            var variable = _parser.GetVariableRef(varName);
            ...
            else if (_isVisible)
            {
                try
                {
                    _parser.Parse(expression, true);
                    _sb.Append($"<p{HtmlId}><span class=\"cond\">#const</span> <span class=\"eq\">{_parser.ToHtml()}</span></p>");
                }
                catch (MathParserException ex)
                {
                    AppendError(s.ToString(), ex.Message, _currentLine);
