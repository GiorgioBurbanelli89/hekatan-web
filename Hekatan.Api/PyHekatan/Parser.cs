...
namespace PyHekatan
{
    public class Parser
    {
        ...
        public bool Convert(string inputFileName, string outputFileName)
        {
            ...
            var code = HekatanReader.Read(inputFileName);
            var macroParser = new MacroParser
            {
                Include = HekatanReader.Include
            };
            var hasMacroErrors = macroParser.Parse(code, out var unwrappedCode, null, 0, true);
            string htmlResult;
            Hekatan.Converter converter = new();
            if (hasMacroErrors)
            {
                htmlResult = HekatanReader.CodeToHtml(unwrappedCode);
        ...
        private static Hekatan.Core.Settings ConvertSettings(Settings settings) =>
            new()
            {
        ...
        private static Hekatan.Core.PlotSettings ConvertPlotSettings(PlotSettings settings) =>
            new()
            {
                ...
                ColorScale = (Hekatan.Core.PlotSettings.ColorScales)settings.ColorScale,
                SmoothScale = settings.SmoothScale,
                Shadows = settings.Shadows,
                LightDirection = (Hekatan.Core.PlotSettings.LightDirections)settings.LightDirection
            };
    }
}
