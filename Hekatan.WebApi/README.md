...
Hekatan.WebApi provides the capabilities of Hekatan.Core via API. You can integrate this API into existing projects to achieve web-based calcpad functionality.
...
3. Build the image on the server
    ...
    Navigate to the Hekatan.WebApi directory
...
4. Initial configuration
    ...
    Then refer to `Hekatan.WebApi/appsettings.json` and modify the settings in `appsettings.Production.json` for production configuration. If only for testing, no changes are needed.
...
2. Upload calcpad files through the interfaces in `HekatanFileController.cs`, such as .txt, .cpd, .cpdz
...
HekatanFileController provides functions for file upload, copy, execution, download, etc. Please check the corresponding interfaces for details.
...
Version upgrades can be handled as copying Hekatan files.

Version upgrades involve data migration. Currently, two types are known:

1. User inputs generated via `?`
2. File paths read by `#read` macro (this operation is not owned by HekatanCore, it is a new feature of this system)

To resolve the above two data migrations, the actual migration steps are:

1. Copy a Hekatan file from the new template
