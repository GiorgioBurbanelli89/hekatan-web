...
using Hekatan.Document.Core.Segments;
...
{
    /// <summary>
    /// Hekatan file upload and manage
    /// </summary>
    /// <param name="db"></param>
    ...
    public class HekatanFileController(
        MongoDBContext db,
    ...
    ) : ControllerBaseV1
    {
        ...
        {
            var fileObject = await db.AsQueryable<HekatanFileModel>()
        ...
        /// <returns></returns>
        [HttpPost()]
        public async Task<ResponseResult<string>> UploadHekatanCodeFile(
            [FromForm] UploadHekatanRequestData formData
        )
        {
            ...
            // verify exist
            var existObject = await db.AsQueryable<HekatanFileModel>()
            ...
            if (existObject == null)
            {
                ...
                existObject = new HekatanFileModel
                {
                ...
                await db.Collection<HekatanFileModel>().InsertOneAsync(existObject);
            }
            else
            {
                // updaet is public
                if (existObject.IsCpd != formData.IsCpdFile)
                    await db.AsFluentMongo<HekatanFileModel>()
        ...
        /// <returns></returns>
        [HttpPost("copy")]
        public async Task<ResponseResult<bool>> CopyHekatanCodeFile(string fromId, string toId)
        {
            var existModel = await db.AsQueryable<HekatanFileModel>()
            ...
            // copy model
            var newModel = new HekatanFileModel()
            {
                UniqueId = toId,
            ...
            await db.Collection<HekatanFileModel>().InsertOneAsync(newModel);
        ...
        public async Task<ResponseResult<string>> UpdateHekatanFileMacroRead(
        ...
        /// <returns></returns>
        [HttpDelete("{uniqueId}")]
        public async Task<ResponseResult<bool>> DeleteHekatanFile(
            string uniqueId,
            bool force = false
        )
        {
            ...
            if (force)
            {
                ...
                await db.Collection<HekatanFileModel>().DeleteOneAsync(x => x.Id == existModel.Id);
            }
            else
            {
                await db.AsFluentMongo<HekatanFileModel>()
        ...
        public async Task<IActionResult> GetFileStreamById(string fileId)
        {
            ...
            // query file model
            var fileModel = await db.AsQueryable<HekatanFileModel>()
        ...
        /// <returns></returns>
        private async Task<IActionResult> GetFileStream(HekatanFileModel fileModel)
        {
        ...
        )
        {
            ...
            var pathFileModels = await db.AsQueryable<HekatanFileModel>()
            ...
            foreach (var path in filePaths)
            {
                ...
                if (model == null)
                {
                    ...
                    if (!System.IO.File.Exists(defaultFilePath))
                        continue;
                    model = new HekatanFileModel() { FullName = Path.GetFullPath(defaultFilePath) };
                }
        ...
        {
            // get cpd source file
            var fromFile = await db.AsQueryable<HekatanFileModel>()
            ...
            // get cpd target file
            var toFile = await db.AsQueryable<HekatanFileModel>()
