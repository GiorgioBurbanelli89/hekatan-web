...
using Hekatan.WebApi.Services.AI;
...
namespace Hekatan.WebApi.Controllers
{
    ...
    ) : ControllerBaseV1
    {
        ...
        )
        {
            ...
            var cpdFile = await db.AsQueryable<HekatanFileModel>()
            ...
            if (cpdFile == null)
            {
                return false.ToFailResponse("Hekatan file not found");
            }
            ...
            var existingTrans = await db.AsQueryable<HekatanLangModel>()
        ...
        /// <summary>
        /// Generates internationalization (i18n) language resources for a specified Hekatan file using the provided
        /// language code.
        /// </summary>
        /// <remarks>The operation requires that the AI service is enabled and that the specified Hekatan
        ...
        /// <param name="uniqueId">The unique identifier of the Hekatan file for which to generate i18n language resources.</param>
        ...
        /// <summary>
        /// Translates the most recent calculation result of the specified Hekatan file to the given language.
        /// </summary>
        /// <remarks>If the specified Hekatan file does not exist or has no calculation result, the
        ...
        /// <param name="uniqueId">The unique identifier of the Hekatan file whose calculation result is to be translated.</param>
        ...
        )
        {
            ...
            if (cpdFile == null)
            {
                return "".ToFailResponse("Hekatan file not found");
            }
        ...
        /// <summary>
        /// Retrieves all available language models associated with the specified unique Hekatan file identifier,
        /// including those from its source file if applicable.
        /// </summary>
        /// <param name="uniqueId">The unique identifier of the Hekatan file for which to retrieve language models. Cannot be null or empty.</param>
        /// <returns>A response result containing a list of language models associated with the specified Hekatan file and its
        /// source file. The list is empty if no matching file is found.</returns>
        [HttpGet("{uniqueId}/langs")]
        public async Task<ResponseResult<List<HekatanLangModel>>> GetAllLangsByUniqueId(
            string uniqueId
        )
        {
            var cpdFile = await db.AsQueryable<HekatanFileModel>()
            ...
            if (cpdFile == null)
                return new List<HekatanLangModel>().ToFailResponse("Hekatan file not found");
            var cpdUids = await db.AsQueryable<HekatanFileModel>()
                .Where(x => x.GroupId == cpdFile.GroupId)
                .Where(x => x.IsCpd == true)
                .Select(x => x.UniqueId)
                .ToListAsync();

            var langs = await db.AsQueryable<HekatanLangModel>()
        ...
        )
        {
            ...
            await db.AsFluentMongo<HekatanLangModel>()
        ...
        public async Task<ResponseResult<bool>> DeleteLangById(string langId)
        {
            ...
            var langDoc = await db.AsQueryable<HekatanLangModel>()
                .Where(x => x.Id == langObjId)
                .FirstOrDefaultAsync();
