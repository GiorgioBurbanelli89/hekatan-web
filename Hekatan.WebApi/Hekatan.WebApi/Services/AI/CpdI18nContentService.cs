...
namespace Hekatan.WebApi.Services.AI
{
    ...
    ) : IScopedService
    {
        ...
        /// <summary>
        /// Retrieves the Hekatan file with the specified unique identifier and its associated source Hekatan file, if
        /// available.
        /// </summary>
        /// <remarks>The method returns null for both items in the tuple if no Hekatan file with the
        /// specified unique identifier exists. Only files marked as CPD are considered.</remarks>
        /// <param name="uniqueId">The unique identifier of the Hekatan file to retrieve. Cannot be null or empty.</param>
        /// <returns>A tuple containing the Hekatan file that matches the specified unique identifier and its source Hekatan
        /// file, if one exists. The second item in the tuple is null if the file has no source or the source file
        /// cannot be found.</returns>
        public async Task<Tuple<HekatanFileModel, HekatanFileModel?>> GetSelfAndSourceCpdFile(
            string uniqueId
        )
        {
            ...
            if (cpdFile == null)
            {
                return Tuple.Create<HekatanFileModel, HekatanFileModel?>(cpdFile, null);
            }

            HekatanFileModel? sourceFile = null;
            if (!cpdFile.SourceId.IsEmpty())
            {
                sourceFile = await db.AsQueryable<HekatanFileModel>()
        ...
        )
        {
            ...
            var langDict = new Dictionary<string, HekatanLangModel>();
            foreach (var langModel in langs.OrderBy(x => x.Version))
            {
                if (!langDict.TryGetValue(langModel.Key, out HekatanLangModel? value))
                {
                    value = langModel;
            ...
            foreach (var element in i18nElements)
            {
                ...
                if (langDict.TryGetValue(texts[0], out HekatanLangModel? langModel))
                {
                    // replace node value
