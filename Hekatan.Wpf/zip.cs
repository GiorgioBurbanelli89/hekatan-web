 ...
 * The WPF project now uses the unified implementation from Hekatan.Common.
 * This file is kept for reference only.
 *
 * To use the common implementation, add: using Hekatan.Common;
...
namespace Hekatan.Wpf
{
    ...
    {
        internal static void Compress(string text, Stream fs) =>
            Hekatan.Common.Zip.Compress(text, fs);

        internal static void CompressWithImages(string text, string[] images, string fileName) =>
            Hekatan.Common.Zip.CompressWithImages(text, images, fileName);

        internal static SpanLineEnumerator Decompress(Stream fs) =>
            Hekatan.Common.Zip.Decompress(fs).AsSpan().EnumerateLines();

        internal static string DecompressToString(Stream fs) =>
            Hekatan.Common.Zip.Decompress(fs);

        internal static bool IsComposite(string fileName) =>
            Hekatan.Common.Zip.IsComposite(fileName);

        internal static string DecompressWithImages(string fileName) =>
            Hekatan.Common.Zip.DecompressWithImages(fileName);
    }
}
